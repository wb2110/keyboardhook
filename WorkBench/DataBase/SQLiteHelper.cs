...
namespace System.Data.SQLite
{
    ...
    public class SQLiteHelper
    {
        ...
        private  SQLiteConnection GetConn(string cs) {
        ...
        private SQLiteCommand GetExecuter(SQLiteConnection conn) {
            if (cmd == null)
            {
        ...
        public SQLiteHelper(string cs)
        {
            conn = GetConn(cs);
            cmd = GetExecuter(conn);
        }

        #region DB Info
        public List<String> GetTableColumns(string tablename) {
            var retSet = Select("select * from " + tablename);
            var colNames = new List<string>();
            foreach(DataColumn col in retSet.Columns)
            {
        ...
        public int Insert(string tableName, Dictionary<string, object> dic)
        {
            ...
            foreach (var col  in columns)
            {
                ...
                cmd.Parameters.AddWithValue("@v" + col, dic[col]);
            }
        ...
        public int Update(string tableName, Dictionary<string, object> dicData, string colCond, object varCond)
        {
            ...
            dic[colCond] = varCond;
            if (dicData.ContainsKey(colCond)) {
                dicData.Remove(colCond);
            }
